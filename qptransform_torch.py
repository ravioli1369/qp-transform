...
and its chirplet version, the wavelet Qp-transform, with Gaussian-wavelets with increasing (or possibly decreasing) frequency
Moreover, this code implements the possibility to invert the wavelet Qp-transform 
(and so the wavelet Q-transform as well) with a non standard inversion formula. 
...
# wavelet Qp-transform defaults
DEFAULT_FRANGE = (0.0, float("inf"))
DEFAULT_QRANGE = (2.0 * PI, 20.0 * 2.0 * PI)
DEFAULT_PRANGE = None
DEFAULT_ALPHA = 1.0
...
class QpObject(object):
    """
    Base class for wavelet Qp-transform objects
    This object exists just to provide basic methods for other wavelet Qp-transform objects.
    Parameters
        ----------
    ...
    alpha : the distance in units of Gaussian-sigmas between different wavelet Qp-transform points
        i.e. for a given time-frequency point (tau_0, nu_0) the nearest point in time is at tau_1=tau_0+alpha*sigma_tau(nu=nu_0),
        the nearest point in frequency is at nu_1=nu_0+alpha*sigma_nu(nu=nu_0)
    """
...
class QpTile(QpObject):
    ...
    @property
    def sigma_tau(self):
        return self.Q / (4.0 * PI * self.frequency)

    @property
    def sigma_nu(self):
        return (self.frequency / self.Q) * torch.sqrt(1 + ((2 * self.p * self.Q) ** 2))
    ...
    def ntiles(self):
        ...
        upper frequency covered by Gaussian window (i.e. frequency+NUMBER_OF_SIGMAS*sigma_nu)
        ...
        while (
            ...
            < self.frequency + NUMBER_OF_SIGMAS * self.sigma_nu + 1.0 / self.duration
        ):
            n_tiles *= 2
        return n_tiles
    ...
    def windowsize(self):
        ...
        For Gaussian window it has been decided to integrate up to +-NUMBER_OF_SIGMAS*sigma_nu (default is 3.)
        """
        return 2 * int((NUMBER_OF_SIGMAS * self.sigma_nu) * self.duration)

    def _get_indices(self):
        half = int((self.windowsize) / 2)
        return torch.arange(-half, half + 1, 1, dtype=int)

    def get_data_indices(self):
        """Returns the index array of interesting frequencies for this row"""
        return torch.round(self._get_indices() + self.frequency * self.duration).astype(
            int
        )

    def get_window(self, fft_freqs):
        """Generate the Gaussian window for this row with 'frequency' as central frequency"""
        Qtilde = self.Q / torch.sqrt(1.0 + 2.0j * self.Q * self.p)
        norm = (
            ((2.0 * PI / (self.Q**2)) ** (1.0 / 4.0))
            * torch.sqrt(1.0 / (2.0 * PI * self.frequency))
            * Qtilde
        )
        exp_argument = (
            (0.5 * Qtilde * (fft_freqs - self.frequency)) / self.frequency
        ) ** 2
        return norm * torch.exp(-exp_argument)
    ...
    def transform(
        ...
        # this is to tile time points according to the time resolution at each frequency
        if original_sampling_rate == False:
            ...
            if derivative is True:
                fft_series *= 2.0 * PI * 1.0j * fft_freqs

            windowed = fft_series * self.get_window(fft_freqs)

            # this needs a frequency shift (performed later)
            # transform_fft = numpy.pad(windowed, self.padding, mode="constant")
            pad = (self.padding, self.padding)
            transform_fft = torch.nn.functional.pad(windowed, pad, mode="constant")
            ...
            if start_time is None:
                start_time = 0.0
            # IFFT and return a `TimeSeries`, normalising by sqrt(sampling) (dimensionless version of the transform)
            transform = torch.fft.ifft(transform_fft, norm="backward")
            transform_ts = torch.sqrt(self.sampling) * TimeSeries(
                transform, x0=start_time, dx=self.duration / transform.size, copy=False
            )

            # here we properly shift in frequency
            transform_ts *= torch.exp(
                -2.0j * PI * fft_freqs[0] * transform_ts.times.value
            )
        ...
        # this instead takes the same number of time points at each frequency (i.e. those given by the sampling rate); that is useful for inversion formula and spectrogram representation
            ...
            transform = torch.fft.ifft(windowed, norm="backward")
            transform_ts = torch.sqrt(self.sampling) * TimeSeries(
                transform, x0=start_time, dx=self.duration / transform.size, copy=False
            )
...
class QpPlane(QpObject):
    """
    wavelet Qp-transform plane made by QpTile objects calculated for different frequencies at fixed Q and p
    """
    ...
    def __init__(self, frange, Q, p, duration, sampling, alpha=DEFAULT_ALPHA):
        ...
        lowest_nu = (1.0 / self.duration) * (
            1.0
            - (NUMBER_OF_SIGMAS * torch.sqrt(1 + ((2 * self.p * self.Q) ** 2))) / self.Q
        )
        highest_nu = (self.sampling / 2.0) / (
            1.0
            + (NUMBER_OF_SIGMAS * torch.sqrt(1 + ((2 * self.p * self.Q) ** 2))) / self.Q
        )  # no integration above sampling=2*(frange[-1]+(NUMBER_OF_SIGMAS*numpy.sqrt(1+((2*self.p*self.Q)**2)))*sigma_nu(nu=frange[-1]))

        if self.frange[0] < lowest_nu:  # set non-zero lower frequency
            self.frange[0] = lowest_nu
            warnings.warn(f"lower bound of frequency range set to {lowest_nu} Hz")

        if self.frange[-1] > highest_nu:  # set non-infinite upper frequency
            self.frange[-1] = highest_nu
            warnings.warn(f"upper bound of frequency range set to {highest_nu} Hz")

    @property
    def frequencies(self):
        """
        Given frange and alpha, this finds the frequencies on which the wavelet Qp-transform is calculated
        """
        nfreq = int(
            torch.ceil(
                torch.log(self.frange[1] / self.frange[0])
                / torch.log(
                    1.0
                    + (self.alpha * torch.sqrt(1 + ((2 * self.p * self.Q) ** 2)))
                    / self.Q
                )
            )
        )
        return torch.logspace(
            torch.log10(self.frange[0]), torch.log10(self.frange[1]), nfreq
        )
...
class QpGram(QpObject):
    ...
    Parameters (not inherited by QpObject)
        ----------
    plane : the complex values obtained with the wavelet Qp-transform
    energies : the squared module of the wavelet Qp-transform (i.e. abs(plane)**2)
    frequencies : the frequencies on which the wavelet Qp-transform is calculated
...
class QpTransform:
    ...
    Inputs
    ----------
    data : the data needed for wavelet Qp-transform
    times : the time array corresponding to 'data'
    outseg : the specific time segment on which the wavelet Qp-transform will be applied; if 'None' no cut is applied
    whiten : if 'True' whitening is applied
    derivative : if 'True' the time derivative of the wavelet Qp-transform is calculated in 'qpgram_derivative'
    frange : the frequency range on which the wavelet Qp-transform is calculated
    alpha : the alpha used for calculate the final wavelet Qp-transform (i.e. once Q and p have been found)
    ...
    qpgram : the QpGram object resulting from the wavelet Qp-transform
    ...
    qpspecgram : the Spectrogram (i.e. energies) associated to this wavelet Qp-transform; time pixels are interpolated to 'times'
...
def find_Q(Q, *args):
    """
    Function to find Q maximising the energy density at the energy peak
    """
    (
        p,
        frange,
...
def find_p(p, *args):
    """
    Function to find p maximising the energy density at the energy peak
    """
    (
        Q,
        frange,
...
def qp_scan(
    ...
    Parameters
    ----------
    series : the TimeSeries object with the data needed for wavelet Qp-transform
    times : the time array corresponding to 'series'
    frange : the frequency range on which the wavelet Qp-transform is calculated
    alpha : the alpha used for calculate the final wavelet Qp-transform (i.e. once Q and p have been found)
    alpha_find_Qp : the alpha used for finding the Q and p values throug the maximization of the qpgram energy density;
...
def qpgram_to_qpspecgram(qpgram, to_return):
    ...
    if to_return == "energies":
        qpobject = qpgram.energies

    elif to_return == "plane":
...
def inverse_Qp_transform(qpmatrix_derivatives, times):
    ...
    and returns a corresponding TimeSeries object obtained with the reconstruction formula for the wavelet Qp-transform
    Parameters
    ----------
    ...
    Returns
    ----------
    iqptseries : the TimeSeries object obtained with the reconstruction formula for the wavelet Qp-transform
    """
