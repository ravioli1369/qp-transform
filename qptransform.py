...
class QpTile(QpObject):
    ...
    def ntiles(self):
        ...
        while(0.5*(n_tiles/self.duration)<self.frequency+NUMBER_OF_SIGMAS*self.sigma_nu+1./self.duration): 
            n_tiles*=2
        return n_tiles
    ...
    def _get_indices(self):
        ...
        return numpy.arange(-half, half+1, 1, dtype=int)
...
class QpPlane(QpObject):
    ...
    def __init__(self, frange, Q, p, duration, sampling, alpha=DEFAULT_ALPHA):
        ...
        lowest_nu=(1./self.duration)*(1.-(NUMBER_OF_SIGMAS*numpy.sqrt(1+((2*self.p*self.Q)**2)))/self.Q)
        highest_nu=(self.sampling / 2.)/(1.+(NUMBER_OF_SIGMAS*numpy.sqrt(1+((2*self.p*self.Q)**2)))/self.Q)  # no integration above sampling=2*(frange[-1]+(NUMBER_OF_SIGMAS*numpy.sqrt(1+((2*self.p*self.Q)**2)))*sigma_nu(nu=frange[-1]))
    ...
    if(qrange[0]==qrange[-1] and prange[0]==prange[-1]):
        ...
        if(prange==[0.,0.]):
            ...
            if(prange is not None):
                ...
                Qp_bnds=scipy.optimize.Bounds(lb=[qrange[0],0], ub=[qrange[-1],1./qrange[0]]) 
